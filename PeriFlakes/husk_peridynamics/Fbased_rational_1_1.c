...
void Fbased_rational_1_1_eval(int l_edge,
/*Inputs:*/
...
mK[0]+= alpha[i - 1]*p_Vol[0]*pow(-x[0] + x[2*i], 2)*((sqrt(pow(-x[0] + x[2*i], 2) + pow(-x[1] + x[2*i + 1], 2)) <= delta[0]) ? (
   (1.0 - sqrt(pow(-x[0] + x[2*i], 2) + pow(-x[1] + x[2*i + 1], 2))/delta[0])/sqrt(pow(-x[0] + x[2*i], 2) + pow(-x[1] + x[2*i + 1], 2))
)
: (
   0.0
));
...
: (
   ...
   (1.0 - sqrt(pow(-x[0] + x[2*j], 2) + pow(-x[1] + x[2*j + 1], 2))/delta[0])/sqrt(pow(-x[0] + x[2*j], 2) + pow(-x[1] + x[2*j + 1], 2))
)
: (
   0.0
...
void Fbased_rational_1_1_eval_wr(int l_edge, const real_t * restrict in, real_t * restrict out) {
   Fbased_rational_1_1_eval(l_edge,in, in+(int)(l_edge + 1), in+(int)(l_edge + 1)+(int)(1), in+(int)(l_edge + 1)+(int)(1)+(int)(1), in+(int)(l_edge + 1)+(int)(1)+(int)(1)+(int)(1), in+(int)(l_edge + 1)+(int)(1)+(int)(1)+(int)(1)+(int)(1), in+(int)(l_edge + 1)+(int)(1)+(int)(1)+(int)(1)+(int)(1)+(int)(l_edge + 1), 
out, out+(int)(pow(l_edge + 1, 2)));
}

void kmap_Fbased_rational_1_10(int * edge, int l_edge, int * verts, int * n, int * dim) {
  int i;
  *dim = 2;
...
void kmap_Fbased_rational_1_11(int * edge, int l_edge, int * verts, int * n, int * dim) {
  int i;
  *dim = 1;
  *n = (int)((int)(1));
  if(edge) {
...
void kmap_Fbased_rational_1_12(int * edge, int l_edge, int * verts, int * n, int * dim) {
  int i;
  *dim = 1;
  *n = (int)((int)((1.0L/2.0L)*l_edge - 1.0L/2.0L));
  if(edge) {
...
kernel_t kernel_Fbased_rational_1_1 = {
.nmap = 3,
.maps = {
kmap_Fbased_rational_1_10,
kmap_Fbased_rational_1_11,
kmap_Fbased_rational_1_12
},
...
.eval=Fbased_rational_1_1_eval_wr,
.name="Fbased_rational_1_1"
};

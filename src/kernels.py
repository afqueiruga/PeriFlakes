...
x0,xI,xJ = i_y.Vertex_Handles(0,II,JJ)
...
class StateBased():
    ...
    def theta_expr(self):
        return gdim/m_sym *  self.w0I*alpha_I * ((ryabs-rxabs)* rxabs)
    def force(self):
        A =  alpha_I * self.w0I/m_sym *(  ( 2.0*(c_K-c_G/3.0) * th )* rxabs
                                          + 8.0*c_G * (strainI - th*rxabs/3.0) )
        return A * xn
